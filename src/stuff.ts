...
export function binaryIntoDec(arrayBin: Array<number>): number {
...
export function parseCoordinate(entry: string): Coordinate {
    let splitEntry = entry.split(',');
    return {x: parseInt(splitEntry[0]), y: parseInt(splitEntry[1])};
}

export function getNeighbors(point: Coordinate, width: number, height: number, diag: boolean): Coordinate[] {
...
export function distanceBetween(loc1: Coordinate, loc2: Coordinate): number {
    return Math.sqrt((loc1.x - loc2.x) ** 2 + (loc1.y - loc2.y) ** 2);
}

export function manhattanDistanceBetween(loc1: Coordinate, loc2: Coordinate): number {
...
export function lowestCommonMultiple(a: number, b: number) {
...
console.assert(greatestCommonDivisor(14, 12) === 2);
console.assert(lowestCommonMultiple(12, 8) === 24);
